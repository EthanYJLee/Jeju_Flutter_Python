...
class LoginSignUp {
  ...
  Future<String> login(String uId, String uPassword) async {
    String url =
        'http://${Shared.ipAddress}:8080/login?uId=$uId&uPassword=$uPassword';
  ...
  //Desc: 회원가입
  //Date: 2023-02-21
  Future<bool> join(String uId, String uIdType, String uPassword, String uName,
      String uNickname, String uEmail, String uSex, String uBirth) async {
    String url =
        'http://${Shared.ipAddress}:8080/join?uId=$uId&uIdType=$uIdType&uPassword=$uPassword&uNickname=$uNickname&uEmail=$uEmail&uSex=$uSex&uBirth=$uBirth&uName=$uName';
    var uri = Uri.parse(url);
    http.get(uri);
    return true;
  }
  ...
  Future<String> kakaoCheck(String uId) async {
    String url = 'http://${Shared.ipAddress}:8080/kakaoCheck?uId=$uId';
    var uri = Uri.parse(url);
    var result = await http.get(uri);
    var dataConvertedJson = json.decode(utf8.decode(result.bodyBytes));
    String name = dataConvertedJson['results'][0]['check'];
